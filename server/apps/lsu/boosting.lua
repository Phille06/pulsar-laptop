...
AddEventHandler("Laptop:Server:RegisterCallbacks", function()
    RegisterItems()
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:Admin:CreateContract",
            ...
            if char and data?.vehicle and data?.make and data?.model and data?.trackers and data?.price then
                ...
                if perm["lsunderground"] and perm["lsunderground"]["admin"] then
                    ...
                    if level >= 1 and BOOSTING_VEHICLE_CONFIG[data.class] and cost > 0 then
                        ...
                        cb(exports['pulsar-laptop']:LSUndergroundBoostingGiveContract(source, {
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:Admin:GetBans",
        function(source, data, cb)
            local char = exports['pulsar-characters']:FetchCharacterSource(source)
            if char then
                local perm = char:GetData("LaptopPermissions")
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:Admin:Ban", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:Admin:Unban",
        function(source, data, cb)
            local char = exports['pulsar-characters']:FetchCharacterSource(source)
            if char and data?.SID then
                local perm = char:GetData("LaptopPermissions")
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:EnterQueue", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        ...
        for k, v in ipairs(team.Members) do
            local reputation = exports['pulsar-characters']:RepGetLevel(v.Source, _boostingRequiredRep.rep)
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:ExitQueue", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if char then
            for k, v in ipairs(_boostingQueue) do
                if v.source == source then
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:DeclineContract",
        function(source, data, cb)
            local char = exports['pulsar-characters']:FetchCharacterSource(source)
            if char then
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:TransferContract",
        function(source, data, cb)
            local char = exports['pulsar-characters']:FetchCharacterSource(source)
            if char and data?.alias then
                ...
                for k, v in ipairs(boostingContracts) do
                    if v.id == data.id then
                        local found = nil
                        for _, c in pairs(exports['pulsar-characters']:FetchAllCharacters()) do
                            if c ~= nil then
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:AcceptContract",
        function(source, data, cb)
            local char = exports['pulsar-characters']:FetchCharacterSource(source)
            if char then
                local boostingContracts = char:GetData("BoostingContracts") or {}
                local perm = char:GetData("LaptopPermissions")
                local team, isLeader = exports['pulsar-laptop']:TeamsGetByMemberSource(source)
                local level = exports['pulsar-characters']:RepGetLevel(source, "Boosting")
                if level < 1 then
                    level = 1
                end
                ...
                for k, v in ipairs(boostingContracts) do
                    if v.id == data.id and (v.expires > os.time()) and (level >= v.vehicle.classLevel or v.vehicle.rewarded) then
                        if team?.ID and (#team.Members >= 2 or (perm["lsunderground"] and perm["lsunderground"]["admin"])) then
                            if exports['pulsar-finance']:CryptoHas(source, v.prices.standard.coin, v.prices.standard.price) then
                                ...
                                exports['pulsar-laptop']:AddNotification(team.ID, "New Request for a Boosting Contract",
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:Exterior", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if char then
            ...
            if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 0 then
                ...
                exports['pulsar-laptop']:TeamsMembersSendEvent(team.ID,
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:Ignition", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if char then
            local team = exports['pulsar-laptop']:TeamsGetByMemberSource(source)
            ...
            if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 1 then
                if _boosting[team.ID].trackerTotal and _boosting[team.ID].trackerTotal > 0 then
                    ...
                    exports['pulsar-laptop']:TeamsMembersNotificationAddWithId(
                        team.ID,
                        "BOOSTING_CONTRACT",
                        "Current Contract",
                        string.format("Disable tracking devices inside the vehicle. %s/%s",
                            _boosting[team.ID].trackerCount, _boosting[team.ID].trackerTotal),
                        os.time() * 1000,
                        -1,
                        "lsunderground",
                        {},
                    ...
                    CreateThread(function()
                        ...
                        while _boosting[team] and _boosting[team].state == 2 do
                            if DoesEntityExist(_boosting[team].vehicle) then
                                ...
                                TriggerClientEvent("EmergencyAlerts:Client:TrackerBlip", -1, "police",
                                    string.format("boosting-%s", team),
                                    string.format("[Police]: Class %s Boost (From %s)", _boosting[team].vehicleData
                                        .class, location), coords, 523, 6, 1.0, true, 155)
                            end
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:DropOff", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if char then
            local team = exports['pulsar-laptop']:TeamsGetByMemberSource(source)
    ...
    exports["pulsar-core"]:RegisterServerCallback("Laptop:LSUnderground:Boosting:LeftArea", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if char then
            local team = exports['pulsar-laptop']:TeamsGetByMemberSource(source)
            ...
            if team?.ID and _boosting[team.ID] and _boosting[team.ID].state == 4 then
                ...
                if not stillInZone then
                    if #(GetEntityCoords(_boosting[team.ID].vehicle) - _boosting[team.ID].dropOff) <= 30.0 then
                        exports['pulsar-laptop']:LSUndergroundBoostingComplete(team?.ID)
                    else
                        exports['pulsar-laptop']:TeamsMembersNotification(
                            team.ID,
                            "Current Contract",
    ...
    exports.ox_inventory:RegisterUse("boosting_tracking_disabler", "Boosting", function(source, slot, itemData)
        ...
        then
            SetTimeout(500, function()
                exports["pulsar-core"]:ClientCallback(source, "Laptop:LSUnderground:Boosting:TrackerHacker", {},
            ...
            exports['pulsar-hud']:Notification(source, "error", "Can't Use Right Now...")
        end
    end)
end
...
AddEventHandler("Laptop:Server:LSUnderground:Boosting:ActionRequest", function(source, data, action)
    if action == "accept" and data?.requester and data?.contract then
        local owner = exports['pulsar-characters']:FetchCharacterSource(data.requester)
        if not owner then return; end

        local team, isLeader = exports['pulsar-laptop']:TeamsGetByMemberSource(data.requester)
        ...
        for k, v in ipairs(contracts) do
            if v.id == data.contract then
                if team?.ID and team.State == 0 and (#team.Members >= 2 or (perm["lsunderground"] and perm["lsunderground"]["admin"])) then
                    ...
                    if not fail then
                        if exports['pulsar-finance']:CryptoExchangeRemove(v.prices.standard.coin, owner:GetData("CryptoWallet"), v.prices.standard.price) then
                            table.remove(contracts, k)
                            updated = true

                            exports['pulsar-laptop']:LSUndergroundBoostingStart(team.ID, v)
                        else
                            for _, m in ipairs(team.Members) do
                                exports['pulsar-laptop']:AddNotification(m.Source, "Failed to Start Boosting Contract",
...
exports('LSUndergroundBoostingRewardContract', function(source)
    ...
    if char then
        ...
        local boostingClass = exports['pulsar-core']:UtilsWeightedRandom(_boostingRepToClassChances[level])
        ...
        return exports['pulsar-laptop']:LSUndergroundBoostingGiveContract(source, {
            vehicle = veh.vehicle,
...
exports('LSUndergroundBoostingGiveContract', function(source, vehicle, prices, timeLength, settings)
    ...
    if char then
        ...
        exports['pulsar-core']:LoggerInfo("Boosting",
...
exports('LSUndergroundBoostingStart', function(teamId, contract)
    local team = exports['pulsar-laptop']:TeamsGet(teamId)
    if team and contract and contract?.vehicle?.vehicle then
        ...
        for k, v in ipairs(team.Members) do
            exports['pulsar-characters']:RepRemove(v.Source, "Boosting", takenRep)

            if v.SID == contract.owner?.SID then
                local cChar = exports['pulsar-characters']:FetchCharacterSource(v.Source)
                if cChar then
                    exports['pulsar-core']:LoggerInfo("Boosting",
        ...
        exports['pulsar-laptop']:TeamsSetState(team.ID, "boosting",
        ...
        exports['pulsar-vehicles']:SpawnTemp(source, GetHashKey(contract.vehicle.vehicle), 'automobile',
            ...
            pickUpLocation.coords.w, function(spawnedVehicle, VIN, plate)
                if spawnedVehicle then
                    ...
                    exports['pulsar-laptop']:TeamsMembersSendEvent(team.ID, "Laptop:Client:LSUnderground:Boosting:Start",
                        _boosting[team.ID])
                    ...
                    exports['pulsar-core']:LoggerError("Boosting",
...
exports('LSUndergroundBoostingCancel', function(teamId, teamDeleted)
    if _boosting[teamId] then
        if not teamDeleted then
            exports['pulsar-laptop']:TeamsResetState(teamId)
            exports['pulsar-laptop']:TeamsMembersNotificationRemoveById(teamId, "BOOSTING_CONTRACT")
            exports['pulsar-laptop']:TeamsMembersNotification(
                teamId,
                "Contract Cancelled",
            ...
            exports['pulsar-laptop']:TeamsMembersSendEvent(teamId, "Laptop:Client:LSUnderground:Boosting:End", true)
        end
    ...
    if _boosting[teamId] then
        ...
        if DoesEntityExist(veh) then
            ...
            SetTimeout(60 * 1000, function()
                exports['pulsar-vehicles']:Delete(veh, function(success) end)
            end)
        end

        exports['pulsar-laptop']:TeamsResetState(teamId)
        exports['pulsar-laptop']:TeamsMembersNotificationRemoveById(teamId, "BOOSTING_CONTRACT")
        exports['pulsar-laptop']:TeamsMembersSendEvent(teamId, "Laptop:Client:LSUnderground:Boosting:End")
        TriggerClientEvent("EmergencyAlerts:Client:TrackerBlip", -1, "police", string.format("boosting-%s", teamId))
        _boosting[teamId] = nil
    end
end)
...
AddEventHandler("Laptop:Server:Teams:MemberRemoved", function(teamId, member)
    ...
    if team and team.Members and #team.Members < 2 then -- No longer enough people, kick 'em out of the queue
        ...
        if _boosting[teamId] then
            exports['pulsar-laptop']:LSUndergroundBoostingCancel(teamId)
        end
    end
end)
...
AddEventHandler("Laptop:Server:Teams:Deleted", function(teamId)
    ...
    if _boosting[teamId] then
        exports['pulsar-laptop']:LSUndergroundBoostingCancel(teamId, true)
    end
end)
...
AddEventHandler("Vehicles:Server:Deleted", function(veh)
    for k, v in pairs(_boosting) do
        if v and v.vehicle == veh then
            exports['pulsar-laptop']:LSUndergroundBoostingCancel(k)
        end
    end
end)
...
function SetupBoostingQueue()
    ...
    CreateThread(function()
        ...
        exports['pulsar-core']:LoggerInfo("Boosting", "Boosting Contracts Can Now Be Rewarded")
        ...
        while true do
            ...
            if #_boostingQueue > 0 then
                ...
                if _boostingQueue[index] then
                    local char = exports['pulsar-characters']:FetchCharacterSource(_boostingQueue[index].source)
                    if char then
                        ...
                        if not _boosting[_boostingQueue[index].team] and (not _boostingContractCount[char:GetData("SID")] or (_boostingContractCount[char:GetData("SID")] < BOOSTING_MAX_CONTRACTS)) and (contractCount < BOOSTING_MAX_QUEUE) then
                            ...
                            exports['pulsar-laptop']:LSUndergroundBoostingRewardContract(char:GetData("Source"))
                        end
                    end
                end
